 ...
 */
public final class Long2ReferenceMap<V> {
    ...
    static {
        try {
            ...
            HOT = lookup.findVarHandle(Long2ReferenceMap.class, "hot", HotWindow.class);
    ...
    public Long2ReferenceMap() {
        this(new Builder());
    }

    public Long2ReferenceMap(long hotBaseKey, long hotCapacity) {
        this(new Builder().hotRange(hotBaseKey, hotCapacity));
    }

    public Long2ReferenceMap(long hotBaseKey, long hotCapacity, int pageBits) {
        this(new Builder().hotRange(hotBaseKey, hotCapacity).pageBits(pageBits));
    }

    private Long2ReferenceMap(Builder builder) {
        this.pageBits = builder.pageBits;
        this.pageSize = 1 << pageBits;
        this.pageMask = pageSize - 1;
        this.prefillHotWindow = builder.prefillHotWindow;
        this.vacuumOnEmptyPage = builder.vacuumOnEmptyPage;
        this.mapViewEnabled = builder.mapViewEnabled;
        this.store = new RadixPageStore();
        this.hot = buildHotWindow(builder.hotBaseKey, builder.hotCapacity);
        this.mapView = mapViewEnabled ? new MapView() : null;
    }

    public static <V> Long2ReferenceMap<V> concurrent() {
        return new Long2ReferenceMap<>();
    }

    public static <V> Long2ReferenceMap<V> concurrent(long hotBaseKey, long hotCapacity) {
        return new Long2ReferenceMap<>(hotBaseKey, hotCapacity);
    }
    ...
    public Long2ReferenceMapStats stats() {
        ...
        long estimatedBytes = allocatedPages * (long) pageSize * Long2ReferenceFactory.referenceScale()
                + radixNodes * (long) RADIX_SIZE * Long2ReferenceFactory.referenceScale()
                + window.pageCount * (long) Long2ReferenceFactory.referenceScale();
        return new Long2ReferenceMapStats(
                mappingCount(),
                allocatedPages,
                retiredPages,
                radixNodes,
    ...
    public record Long2ReferenceMapStats(
            long mappingCount,
            long allocatedPages,
            long retiredPages,
            long radixNodes,
    ...
    public static final class Builder {
        private long hotBaseKey;
        private long hotCapacity;
        private int pageBits = DEFAULT_PAGE_BITS;
        private boolean prefillHotWindow = true;
        private boolean vacuumOnEmptyPage = true;
        private boolean mapViewEnabled = true;

        private Builder() {
        }

        public Builder hotRange(long baseKey, long capacity) {
            if (capacity < 0) {
                throw new IllegalArgumentException("capacity must be non-negative");
            }
            Math.addExact(baseKey, capacity);
            this.hotBaseKey = baseKey;
            this.hotCapacity = capacity;
            return this;
        }

        public Builder pageBits(int bits) {
            if (bits < MIN_PAGE_BITS || bits > MAX_PAGE_BITS) {
        ...
        public Builder prefillHotWindow(boolean enabled) {
            this.prefillHotWindow = enabled;
            return this;
        }

        public Builder vacuumOnEmptyPage(boolean enabled) {
            this.vacuumOnEmptyPage = enabled;
            return this;
        }

        public Builder mapViewEnabled(boolean enabled) {
            this.mapViewEnabled = enabled;
            return this;
        }

        public <V> Long2ReferenceMap<V> build() {
            return new Long2ReferenceMap<>(this);
        }
    }
    ...
    private final class EntryIterator implements Iterator<Entry<V>> {
        ...
        EntryIterator() {
            ...
            Long2ReferenceMap.this.forEach((key, value) -> entries.add(new Entry<>(key, value)));
    ...
    private final class MapView extends AbstractMap<Long, V> implements ConcurrentMap<Long, V> {
        ...
        public V get(Object key) {
            return key instanceof Long longKey ? Long2ReferenceMap.this.get(longKey) : null;
        }

        @Override
        public boolean containsKey(Object key) {
            return key instanceof Long longKey && Long2ReferenceMap.this.containsKey(longKey);
        }

        @Override
        public V put(Long key, V value) {
            Objects.requireNonNull(key);
            return Long2ReferenceMap.this.put(key, value);
        }

        @Override
        public V remove(Object key) {
            return key instanceof Long longKey ? Long2ReferenceMap.this.remove(longKey) : null;
        }
        ...
        public boolean remove(Object key, Object value) {
            ...
            @SuppressWarnings("unchecked")
            V typed = (V) value;
            return Long2ReferenceMap.this.remove(longKey, typed);
        }

        @Override
        public boolean replace(Long key, V oldValue, V newValue) {
            Objects.requireNonNull(key);
            return Long2ReferenceMap.this.replace(key, oldValue, newValue);
        }

        @Override
        public V replace(Long key, V value) {
            Objects.requireNonNull(key);
            return Long2ReferenceMap.this.replace(key, value);
        }

        @Override
        public V putIfAbsent(Long key, V value) {
            Objects.requireNonNull(key);
            return Long2ReferenceMap.this.putIfAbsent(key, value);
        }

        @Override
        public void clear() {
            Long2ReferenceMap.this.clear();
        }

        @Override
        public int size() {
            return Long2ReferenceMap.this.size();
        }

        @Override
        public boolean isEmpty() {
            return Long2ReferenceMap.this.isEmpty();
        }
        ...
        public void forEach(BiConsumer<? super Long, ? super V> action) {
            Objects.requireNonNull(action);
            Long2ReferenceMap.this.forEach(action::accept);
        }
        ...
        public V computeIfAbsent(Long key, Function<? super Long, ? extends V> mappingFunction) {
            Objects.requireNonNull(key);
            Objects.requireNonNull(mappingFunction);
            return Long2ReferenceMap.this.computeIfAbsent(key, mappingFunction::apply);
        }
        ...
        public V computeIfPresent(Long key, BiFunction<? super Long, ? super V, ? extends V> remappingFunction) {
            Objects.requireNonNull(key);
            Objects.requireNonNull(remappingFunction);
            return Long2ReferenceMap.this.computeIfPresent(key, remappingFunction::apply);
        }
        ...
        public V compute(Long key, BiFunction<? super Long, ? super V, ? extends V> remappingFunction) {
            Objects.requireNonNull(key);
            Objects.requireNonNull(remappingFunction);
            return Long2ReferenceMap.this.compute(key, remappingFunction::apply);
        }
        ...
        public V merge(Long key, V value, BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
            Objects.requireNonNull(key);
            Objects.requireNonNull(remappingFunction);
            return Long2ReferenceMap.this.merge(key, value, remappingFunction::apply);
        }
    }
    ...
    private final class EntrySetView extends AbstractSet<Map.Entry<Long, V>> {
        ...
        public Iterator<Map.Entry<Long, V>> iterator() {
            Iterator<Entry<V>> iterator = Long2ReferenceMap.this.iterator();
            return new Iterator<>() {
                @Override
                public boolean hasNext() {
